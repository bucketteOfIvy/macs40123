...
def print_return_dict(rd: dict={}):
    ...
    for var in rd.keys():
        ...
        {var}:
        \tTreatment:
        \t  Mean Treatment: {internal['mean_treat']}
        \t  Median Treatment: {internal['median_treat']},
        \t  STD Treatment: {internal['std_treat']}
        \t  n treatment: {internal['n_treat']}
        \tControl:
        \t  Mean Control: {internal['mean_control']}
        \t  Median Control: {internal['median_control']},
        \t  STD Control: {internal['std_control']}
        \t  n control: {internal['n_control']}
        \tDifferences:
        \t  Mean Difference: {internal['mean_diff']}
        \t  Median Difference: {internal['mean_diff']}
        ----------------------------------------------
        """
    print(to_print)
...
def make_comparisons(treatment: pd.DataFrame, control: pd.DataFrame, 
    ...
    for var in comparison_vars:
        ...
        n_treat, n_control = treatment.shape[0], control.shape[0]
    ...
    if 'deathCrashes' in comparison_vars and 'numCrashes' in comparison_vars:
        calc_and_print_odds_ratios(treatment.index, control_obs, nyc_data, 
                                   'deathCrashes', 'numCrashes')
