...
def run_pca(pca_features, min_components, num_features, result_prefix=''):
    ...
    while not explains_enough_variance: 
        ...
        if total_explained_variance >= 0.8 or expl_vars[-1] < 1/num_features:
...
financial_pca_cols = ["medHouseIncome", "unemplRate", 'medHouseVal', 'perOwnerOcc', 'workTravelAv']
med_fric_pca_cols = ['percentNoHs', 'percentBach', 'perBlack', 'perWithoutIns']
...
run_pca(pca_features, 2, "", 5)
run_pca(fin_pca_features, 2, "finance", 5)
run_pca(medfric_pca_features, 2, "medical_friction", 4)